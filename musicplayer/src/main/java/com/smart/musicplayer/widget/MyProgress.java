...
public class MyProgress extends RelativeLayout {
    ...
    private void init(AttributeSet attrs){
    	...
    	for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.myMagicProgress_circleProgressRadus) {//				Log.e("tag", "typedArray.getDimension(attr, 0)="+typedArray.getDimension(attr, 0)+",typedArray.getDimension(i, 0)="+typedArray.getDimension(i, 0));
                circleProgressRadus = typedArray.getDimension(attr, 0);

            } else if (attr == R.styleable.myMagicProgress_horizonalProgressHeight) {
                horizonalProgressHeight = typedArray.getDimension(attr, 0);

            } else if (attr == R.styleable.myMagicProgress_textProgressSize) {
                textProgressSize = typedArray.getDimension(attr, 0);

            } else if (attr == R.styleable.myMagicProgress_colorProgress) {
                colorProgress = typedArray.getColor(attr, Color.GRAY);

            } else if (attr == R.styleable.myMagicProgress_colorSecondProgress) {
